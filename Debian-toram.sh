...
if ! mountpoint -q "$PERSIST_MOUNT"; then
    ...
    if [[ -n "$PART" ]] && cryptsetup isLuks "$PART" 2>/dev/null; then
        if [[ ! -b /dev/mapper/worktop-persist ]]; then
            # Provide visual separation from initramfs output to make the prompt clear
            plymouth quit 2>/dev/null || true
            echo -ne "\n\n=== Worktop Persistence ===\n\n" > /dev/console 2>/dev/null || true
            for i in {1..3}; do
                systemd-ask-password "Enter LUKS Password for Worktop Persist:" | cryptsetup luksOpen "$PART" worktop-persist -d - && break
            done
        fi
...
FSTAB
echo "worktop-persist UUID=$PERSIST_UUID none luks,discard" > /mnt/etc/crypttab

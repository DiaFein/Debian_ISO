...
if [[ ! -d "$UPPER_DIR" ]]; then
    echo "Overlay directory gone before sync could run — persistence not saved." >&2
    exit 1
fi
...
if ! mountpoint -q "$PERSIST_MOUNT"; then
    ...
    if [[ -n "$PART" ]] && cryptsetup isLuks "$PART" 2>/dev/null; then
        if [[ ! -b /dev/mapper/worktop-persist ]]; then
            # Write prompt directly to console - systemd-ask-password is invisible in service context
            plymouth quit 2>/dev/null || true
            echo -e "\n\n=== Worktop Persistence ===" > /dev/console
            LUKS_OPENED=0
            for i in {1..3}; do
                echo -n "Enter LUKS Password for Worktop Persist (attempt $i/3): " > /dev/console
                read -r -s LUKS_PASS < /dev/console
                echo "" > /dev/console
                if echo "$LUKS_PASS" | cryptsetup luksOpen "$PART" worktop-persist -d - 2>/dev/null; then
                    LUKS_OPENED=1
                    break
                fi
...
if [[ "${1:-}" == "--restore" ]]; then
    ...
    # Save overlay path now while it is guaranteed to exist
...
flock -w 300 /run/worktop-sync.lock rsync -aAX --delete --exclude='/tmp/*' --exclude='/var/tmp/*' --exclude='/run/*' --exclude='/proc/*' --exclude='/sys/*' --exclude='/dev/*' --exclude='/mnt/*' --exclude='/media/*' --exclude='/live/*' "$UPPER_DIR/" "${PERSIST_MOUNT}/upper/"
sync
SCRIPT
...
# Stop before live-boot tears down the overlay and before unmount sweep
Before=umount.target
# Run even during reboot/shutdown sequences

...
apt-get update
# Ensure kernel is installed, fallback if backports fails
...
if [[ $IS_LIVE -eq 0 ]]; then
    echo "--> Ensuring host initramfs is updated (Base System)..."
    update-initramfs -u -k all || true
    
    echo "--> Generating clean initramfs for Toram Live..."
    # Hide crypttab temporarily to build clean initramfs for Toram
...
# Create clean files to inject into the live system squashfs
CLEAN_CRYPTTAB=$(mktemp)
CLEAN_FSTAB=$(mktemp)
echo "# Toram Live: No crypttab required" > "$CLEAN_CRYPTTAB"
cat > "$CLEAN_FSTAB" << EOF
# Toram Live FSTAB (Minimal)
...
# Explicitly exclude the host's crypto configs from being captured
echo "etc/crypttab" >> "$EXCLUDE_FILE"
...
mksquashfs / "$LIVE_DIR/filesystem.squashfs.tmp" \
    ...
    -p "etc/crypttab f 644 0 0 < $CLEAN_CRYPTTAB" \
    -p "etc/fstab f 644 0 0 < $CLEAN_FSTAB" \
    || { echo "ERROR: mksquashfs failed."; rm -f "$EXCLUDE_FILE" "$CLEAN_CRYPTTAB" "$CLEAN_FSTAB"; exit 1; }

rm -f "$EXCLUDE_FILE" "$CLEAN_CRYPTTAB" "$CLEAN_FSTAB"
...
# In live mode, ensure crypttab is clean before building initramfs.
# Persistence overlay may have introduced base system crypto entries.
if [[ $IS_LIVE -eq 1 ]]; then
    if grep -qE "^[^#]" /etc/crypttab 2>/dev/null; then
        echo "    Cleaning crypttab for live initramfs generation..."
        cp /etc/crypttab /etc/crypttab.live.bak
        echo "# Toram Live: No crypttab required" > /etc/crypttab
        CLEANED_LIVE_CRYPTTAB=1
    fi
fi
...
for kernel in /boot/vmlinuz-*; do
    ...
    if [[ $IS_LIVE -eq 1 ]]; then
        # Live mode: /boot has the base system's initramfs with crypto baked in.
        # Build a fresh clean initramfs using the live system's clean crypttab.
        # This avoids overwriting the base system's /boot/initrd.img-* files.
        echo "    Building clean initramfs for $version..."
        mkinitramfs -o "$KERNELS_DIR/$version/initrd.img" "$version"
    else
        # Base system mode: /boot already has clean initramfs (crypttab was hidden
        # in Step 0 above). Safe to copy directly.
...
# Restore live crypttab if it was cleaned
if [[ "${CLEANED_LIVE_CRYPTTAB:-0}" -eq 1 ]]; then
    mv /etc/crypttab.live.bak /etc/crypttab
fi

if [[ $KERNEL_FOUND -eq 1 ]]; then
    # Update default live kernels to the newest
    NEWEST_VMLINUZ=$(ls -v /boot/vmlinuz-* | tail -n 1)
    NEWEST_VERSION=$(basename "$NEWEST_VMLINUZ" | sed 's/vmlinuz-//')
    cp "$NEWEST_VMLINUZ" "$LIVE_DIR/vmlinuz"
    # Use the already-built clean initramfs from kernels dir
    cp "$KERNELS_DIR/$NEWEST_VERSION/initrd.img" "$LIVE_DIR/initrd.img"
...
if [[ $IS_LIVE -eq 0 ]] && [[ "${CLEANED_CRYPTTAB:-0}" -eq 1 ]]; then
    echo "--> Restoring host encryption state (Base System)..."
    mv /etc/crypttab.bak /etc/crypttab
    update-initramfs -u -k all || true
fi
...
parted -s "$DISK" mkpart WORKTOP_ROOT 4609MiB "${ROOT_END}MiB"
...
echo "--> Encrypting Base System Root..."
# Use LUKS2 for the root partition
cryptsetup luksFormat --type luks2 "$ROOT_PART" </dev/tty
cryptsetup luksOpen "$ROOT_PART" worktop_root </dev/tty
mkfs.ext4 -L WORKTOP_ROOT /dev/mapper/worktop_root
...
echo "--> Cloning system..."
mount /dev/mapper/worktop_root /mnt
...
# Setup Crypttab for the installed system
echo "worktop_root UUID=$ROOT_PART_UUID none luks,discard" > /mnt/etc/crypttab

# Setup Fstab using the mapper device for root
cat > /mnt/etc/fstab << FSTAB
/dev/mapper/worktop_root / ext4 errors=remount-ro 0 1
...
# Step 1: Build initramfs WITH crypto for base system normal boot
chroot /mnt /usr/bin/env "$ENV" update-initramfs -u
...
# Step 3: Build CLEAN initramfs for live-toram (without base system crypto)
# The base initramfs embeds /etc/crypttab which makes live-toram try to unlock
# the base root. Hide crypttab and rebuild so live-toram only unlocks persistence.
echo "--> Building clean initramfs for Toram Live boot..."
chroot /mnt /bin/bash -c "
    cp /etc/crypttab /etc/crypttab.installer.bak
    echo '# Toram Live: No crypttab required' > /etc/crypttab
    update-initramfs -u -k all
"

# Step 4: Copy CLEAN kernels + initramfs to live directory
...
# Step 5: Restore crypttab and rebuild initramfs for base system boot
echo "--> Restoring base system crypto configuration..."
chroot /mnt /bin/bash -c "
    mv /etc/crypttab.installer.bak /etc/crypttab
    update-initramfs -u -k all
"
...
# Create clean files for the initial ISO squashfs
CLEAN_CRYPTTAB=$(mktemp)
CLEAN_FSTAB=$(mktemp)
echo "# Toram Live: No crypttab required" > "$CLEAN_CRYPTTAB"
cat > "$CLEAN_FSTAB" << EOF
...
mksquashfs "$CHROOT_DIR" "$IMAGE_DIR/live/filesystem.squashfs" \
    ...
    -e boot/live var/cache/apt/archives etc/crypttab etc/fstab \
    -p "etc/crypttab f 644 0 0 < $CLEAN_CRYPTTAB" \
    -p "etc/fstab f 644 0 0 < $CLEAN_FSTAB"

rm -f "$CLEAN_CRYPTTAB" "$CLEAN_FSTAB"
